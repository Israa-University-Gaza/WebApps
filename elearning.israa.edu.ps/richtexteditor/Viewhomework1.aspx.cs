    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (dt_answer.Rows.Count > 0)
            {
                ...
                Editor2.Content = dt_answer.Rows[0]["det"].ToString();
...
edit.Visible = false;
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile)
        {
            ...
            if (ext.ToLower().Equals("ppt") || ext.ToLower().Equals("pptx") || ext.ToLower().Equals("pdf") || ext.ToLower().Equals("doc") || ext.ToLower().Equals("docx") || ext.ToLower().Equals("xls") || ext.ToLower().Equals("xlsx") || ext.ToLower().Equals("jpg") || ext.ToLower().Equals("jpeg") || ext.ToLower().Equals("png"))
            {
                if (FileUpload1.PostedFile.ContentLength <= 5242880)
                {
                    ...
                    r.e_HomeWork_answer_answer_add(Request.QueryString["id"], Session["log"].ToString(), Session["name"].ToString(), Editor2.Content,image);
        ...
        {
            r.e_HomeWork_answer_answer_add(Request.QueryString["id"], Session["log"].ToString(), Session["name"].ToString(), Editor2.Content, "");
            homework.Visible = false;
            Done.Visible = true;
    ...
    {
        r.e_comments_add(Session["log"].ToString(), Session["name"].ToString(), Editor1.Content, Request.QueryString["id"], "how");
