<%@ Page Title="" Language="C#" MasterPageFile="~/Instructor/MasterPage.master" AutoEventWireup="true" CodeFile="ViewVideo.aspx.cs" validateRequest="false" Inherits="Instructor_ViewAnnouncements" %>
...
<p class="text-center">
    ...
    <iframe runat="server" id="fram" width="100%" src="https://www.youtube.com/embed/k4c1TkZZCbQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="height: 440px"></iframe>
   
</p>
   <div class="well">
<h3><span style="font-size:18px;" class="glyphicon glyphicon-eye-open"></span>&nbsp;&nbsp;   الأشخاص الذين شاهدوا هذا المحتوى </h3>
<p class="text-center">
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:conn %>" SelectCommand="SELECT [std_id], [name], [dt],  ROW_NUMBER() OVER (ORDER BY dt desc) row_num FROM [seen] WHERE (([typ] = @typ) AND ([re_id] = @re_id)) ORDER BY dt desc">
        <SelectParameters>
            <asp:Parameter DefaultValue="vid" Name="typ" Type="String" />
