...
public partial class Instructor_Announcements : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                DataTable dt_h = r.e_homework_get1(Request.QueryString["id"]);
                DropDownList2.SelectedValue=dt_h.Rows[0]["crs_id"] + " شعبة ." + dt_h.Rows[0]["section"];
                DropDownList1.SelectedValue = dt_h.Rows[0]["week_id"].ToString();
                deadline.Text = dt_h.Rows[0]["DeadLine"].ToString();
               Editor1.Text = dt_h.Rows[0]["det"].ToString();
                mark.Text = dt_h.Rows[0]["mark"].ToString();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        DateTime dob = DateTime.Parse(Request.Form[deadline.UniqueID]);
        string image = "";
        ...
        if (CheckBox1.Checked)
        {
            ...
            {
                r.e_HomeWork_add(DropDownList2.SelectedValue.ToString().Split('.')[0].Split(' ')[0].Split(' ')[0], DropDownList1.SelectedValue, dob.ToString(),Editor1.Text, dt_sec.Rows[i][1].ToString(), CheckBox1.Checked, Session["log"].ToString(), image,mark.Text);
                hw.Visible = false;
                Done.Visible = true;
            }
        ...
        else
        {
            if (Request.QueryString["id"] != null)
            {
                r.e_HomeWork_Edit(Request.QueryString["id"], DropDownList1.SelectedValue, dob.ToString(), Editor1.Text, image,mark.Text);
                hw.Visible = false;
                Done.Visible = true;
            }
            else
            {
                ...
                r.e_HomeWork_add(DropDownList2.SelectedValue.ToString().Split('.')[0].Split(' ')[0].Split(' ')[0], DropDownList1.SelectedValue, dob.ToString(),Editor1.Text, DropDownList2.SelectedValue.ToString().Split('.')[1], CheckBox1.Checked, Session["log"].ToString(), image, mark.Text);
            hw.Visible = false;
            Done.Visible = true;
            }
        }
       ...
       // r.e_HomeWork_add(DropDownList2.SelectedValue, DropDownList1.SelectedValue, dob.ToString(),Editor1.Content, DropDownList2.SelectedItem.Text.ToString().Split('.')[1]);
