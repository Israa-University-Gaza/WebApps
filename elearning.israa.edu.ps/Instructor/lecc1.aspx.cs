...
public partial class Instructor_lecc : System.Web.UI.Page
{
    ...
    protected void g2_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (DataBinder.Eval(e.Row.DataItem, "courseCode") != null)
        {
            ...
            SqlDataAdapter DA = new SqlDataAdapter("select count(*) from e_files where crs_id='" + crs_id + "' and section=" + sectionNum, conn);
            DataTable DT = new DataTable();
            DA.Fill(DT);
            try
            {
    ...
    {
        Response.Clear();
        Response.AddHeader("content-disposition", "attachment;filename=Contracts" + DateTime.Now.ToString("dd-MM-yyyy") + ".xls");
        Response.Charset = "";
