    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (CheckBox1.Checked)
        {
            ...
            for (int i = 0; i < dt_sec.Rows.Count; i++)
            {
                ...
                r.e_Announcements_add(tit.Text, DropDownList2.SelectedValue.ToString().Split('.')[0].Split(' ')[0], DropDownList1.SelectedValue, Editor1.Text, dt_sec.Rows[i][1].ToString(), CheckBox1.Checked, Session["log"].ToString());
                ann.Visible = false;
                Done.Visible = true;
            }
        ...
        {
r.e_Announcements_add(tit.Text, DropDownList2.SelectedValue.ToString().Split('.')[0].Split(' ')[0], DropDownList1.SelectedValue, Editor1.Text, DropDownList2.SelectedValue.ToString().Split('.')[1], CheckBox1.Checked, Session["log"].ToString());
            ann.Visible = false;
            Done.Visible = true;
        }
       ...
       // r.e_Announcements_add(tit.Text, DropDownList2.SelectedValue, DropDownList1.SelectedValue, Editor2.Text, DropDownList2.SelectedItem.Text.ToString().Split('.')[1]);
