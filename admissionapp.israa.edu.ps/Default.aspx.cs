...
public partial class AcademicApp_Default : UserPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            DataTable DT = DA.StudentTawjehyDataGet(StudentID);
            if (DT.Rows.Count > 0)
            {
                student_Num.Text = UserPage.StudentNo;
                ...
                divStudentType234.Visible = btnSave.Visible = ((DT.Rows[0]["StudentTypeID"].ToString()) != "1" && (DT.Rows[0]["StudentTypeID"].ToString() != "5")); 
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            if (DA.StudentTawjehyDataAddEdit(StudentID, txtOldUniversity.Text, txtOldDepartment.Text, txtOldUniversityHours.Text, txtOldUniversityGPA.Text, txtShamelRate.Text, txtExpertYears.Text) > 0)
            {
