...
$sql = "SELECT [Transaction].[ID] AS TransactionID
  ...
  $params = array($_GET['TransactionID']);
...
if (count($row)<1){
	...
	sqlsrv_free_stmt( $stmt);
		...
		$detailsTable = "<table style='border: 0px solid #000; width:7cm;border-collapse: collapse;'>
							<tr>
								<td style='border: 0px solid #000;text-align:center;font-weight:bold;width:3cm;'> </td>
								<td style='border: 0px solid #000;text-align:center;font-weight:bold;width:4cm;'> </td>
							</tr>";
		while( $row_5 = sqlsrv_fetch_array( $stmt_5, SQLSRV_FETCH_ASSOC) ) {
			$detailsTable = $detailsTable ."<tr>
				<td style='border: 0px solid #000;text-align:center;font-weight:bold'>".$row_5['Amount']."</td>
				<td style='border: 0px solid #000;text-align:center;font-weight:bold'>".$row_5['CurrencyName']."</td>
			</tr>";
		}
		...
		$decimalToChar = "";
		$fractionToChar = "";
		$decimalAmount = intval($row['Amount']);
		...
		$amountFraction = (floatval($row['Amount']) - intval($row['Amount']));
		if($amountFraction > 0){
			$amountFraction = intval(round($amountFraction * 100));
...
<div class="book">
	...
	<div class="subpage">
			...
			<div style="width: 100;text-align: right;position: relative;top: 94px;right: 618px;"><?PHP echo $row['TransactionID']. "&nbsp;";?></div>
			...
			<div style="width: 100;text-align: right;position: relative;top: 72px;right: 616px;"><?PHP echo date_format($row['InsertDate'],'Y/m/d'). "&nbsp;";?></div>
			<div style="width: 400px;text-align: right;position: relative;top: 126px;right: 147px;"><?PHP echo $row['AccountName']. "&nbsp;";?></div>
			<div style="width: 400px;text-align: right;position: relative;top: 144px;right: 150px;"><?PHP echo $row['Amount']. "&nbsp;دينار أردني";?></div>
			...
			<div style="width: 100;text-align: right;position: relative;top: 362px;right: 618px;"><?PHP echo $row['TransactionID']. "&nbsp;";?></div>
			...
			<div style="width: 100;text-align: right;position: relative;top: 343px;right: 616px;"><?PHP echo date_format($row['InsertDate'],'Y/m/d'). "&nbsp;";?></div>
			<div style="width: 400px;text-align: right;position: relative;top: 403px;right: 147px;"><?PHP echo $row['AccountName']. "&nbsp;";?></div>
			<div style="width: 400px;text-align: right;position: relative;top: 420px;right: 150px;"><?PHP echo $row['Amount']. "&nbsp;دينار أردني";?></div>
