...
public partial class Financial_StudentFromAccount : UserPage
{
    ...
    {
        bool Validate = ValidateControls(new WebControl[] { ddlAction, txtValue, lblStudentID });
        if (Validate)
        {
            DataTable DT = DataAccess.AddStudentFinancialReturnAmount(lblStudentID.Text, ddlAction.SelectedValue, txtValue.Text, EmployeeID);
