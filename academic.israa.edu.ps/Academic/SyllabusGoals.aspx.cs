...
public partial class Academic_SyllabusGoals : UserPage
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Validate)
        {
            DataTable DT = DataAccess.SyllabusGoalAddEdit(lblSyllabusGoalID.Text, lblSyllabusID.Text, txtArGoal.Text, txtEnGoal.Text, EmployeeID, lblAccreditationDepartmentID.Text);
    ...
    protected void lbSyllabusGoalDelete_Click(object sender, EventArgs e)
    {
        ListViewItem li = (ListViewItem)((LinkButton)sender).Parent;
        string id = ((Label)li.FindControl("SyllabusGoalID")).Text;
        DataTable DT = DataAccess.SyllabusGoalDelete(id, EmployeeID, lblAccreditationDepartmentID.Text);
        ShowToast(this, DT.Rows[0]["msg"].ToString(), "", (Convert.ToInt32(DT.Rows[0]["status"].ToString()) > 0) ? "success" : "error");
        ListView1.DataBind();
    }
}