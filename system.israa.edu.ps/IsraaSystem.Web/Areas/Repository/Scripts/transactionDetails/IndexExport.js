...
function DataTableGo() {
    a.init({
        ...
        dataTable: {
            ...
            buttons: [
...

	            {
		            ...
		            init: function (dt, node, config) {
                        $(node).attr('href', '/Repository/TransactionDetails/AddExportDetail?TransactionId=' + $('#TransactionId').val());
		            }
	            }
            ],
            "columns": [
...

                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a href="/Repository/TransactionDetails/DeleteExportDetail/` + data + `" data-toggle="tooltip" data-placement="right" title="" class="btn btn-xs red-sunglo confirm" data-title="حذف" data-msg="هل أنت متأكد من أنك تود الحذف ؟" data-action="حذف" data-ajax-update="#refresh-list" data-original-title="حذف"><i class="fa fa-times"></i> حذف</a>`;
            ...
            ajax: {
                "url": '/Repository/TransactionDetails/LoadExportData?transactionId=',
