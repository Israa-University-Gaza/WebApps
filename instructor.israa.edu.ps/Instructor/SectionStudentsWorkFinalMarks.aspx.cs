...
public partial class Instructor_SectionStudentsFinalMarks : UserPage
{
    ...
    protected void btnSaveMark_Click(object sender, EventArgs e)
    {
        ...
        if (IntValidateControls(new WebControl[] { txtWorkMark, txtFinalMark }))
        {
            DataTable DT = DataAccess.SetStdSectionWorkFinalMark(StdSectionID, txtWorkMark.Text, txtFinalMark.Text, IsFinalNoMark, EmployeeID);
    ...
    {
        /*if(ddlSection.SelectedValue.Length == 0)*/
        ddlSection.DataBind();
        ListView1.DataSource = DataAccess.ListviewResult(ddlSection.SelectedValue).ToList();
        Repeater1.DataBind();
        ListView1.DataBind();
