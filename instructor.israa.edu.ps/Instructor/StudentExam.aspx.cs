...
public partial class Instructor_StudentExam : UserPage
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
            ...
            if (QuestionTypeID != "1" && QuestionTypeID != "2")
            {
                ...
                if (DoubleValidateControls(new WebControl[] { txtStudentMark }))
                {
                    ...
                    if (StudentMark <= QuestionMark)
                        DataAccess.CorrectStudentExamQuestion(lblStudentExamQuestionID.Text, txtStudentMark.Text);
                    else {
                        valid = false;
        ...
        if (valid)
        {
            DataTable DT = DataAccess.CalculateStudentExamMarks(lblStudentExamID.Text);
            Repeater1.DataBind();
            Repeater2.DataBind();
