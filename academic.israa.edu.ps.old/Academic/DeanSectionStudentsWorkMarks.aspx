<%@ Page Title="" Language="C#" MasterPageFile="~/Academic/MasterPage.master" AutoEventWireup="true" CodeFile="DeanSectionStudentsWorkMarks.aspx.cs" Inherits="Academic_DeanSectionStudentsWorkMarks" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        $('document').ready(function () {
            ...
            $('#DeanSectionsWorkMarks_2 a').css("background-color", "#575757");
            $('#DeanSectionsWorkMarks_2 a span.arrow').addClass("open");
            ...
            <div class="portlet box blue">
                ...
                <div class="portlet-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <asp:DropDownList ID="ddlSemester" runat="server" CssClass="select2_category form-control" DataSourceID="SqlDataSource8" DataTextField="ArName" DataValueField="ID" AutoPostBack="true"></asp:DropDownList>
                            <asp:SqlDataSource ID="SqlDataSource8" runat="server" ConnectionString="<%$ ConnectionStrings:isra %>" SelectCommand="SELECT ID, ArName FROM [Semester] Where ID<>1 and IsDelete=0 ORDER BY ID DESC"></asp:SqlDataSource>
                        </div>
                    </div>
                    <div class="row">
                                                ...
                                                <div style="font-size: 14px;" class="alert alert-info alert-dismissable">
                                                    <asp:Repeater ID="Repeater1" runat="server" DataSourceID="SqlDataSource3">
                                                        <ItemTemplate>
                                                            ...
                                                            <div class="col-md-4" style="text-align: center;">
                                                                <br />
                                                                <asp:Button ID="btnMarkAllAccreditation" runat="server" Text='<%# (Convert.ToBoolean(Eval("TeacherAccreditation").ToString()) &&  !Convert.ToBoolean(Eval("DeanAccreditation").ToString()) && !Convert.ToBoolean(Eval("AcademicAccreditation").ToString())) ? "إعتماد كعميد":"تم إعتماد العلامات" %>' CssClass="btn green" Enabled='<%# (Convert.ToBoolean(Eval("TeacherAccreditation").ToString()) &&  !Convert.ToBoolean(Eval("DeanAccreditation").ToString()) && !Convert.ToBoolean(Eval("AcademicAccreditation").ToString()))? true:false %>' OnClick="btnMarkAllAccreditation_Click" OnClientClick="return confirm('هل أنت متأكد من أنك تود إعتماد العلامات ؟');" />
                                                                <asp:Button ID="btnMarkAllUnaccreditation" runat="server" Text="ارجاع العلامات" Enable='<%# (Convert.ToBoolean(Eval("TeacherAccreditation").ToString()) && !Convert.ToBoolean(Eval("AcademicAccreditation").ToString()))? true:false %>' CssClass="btn green" Enabled="true" OnClick="btnMarkAllUnaccreditation_Click" OnClientClick="return confirm('هل أنت متأكد من أنك تود إرجاع العلامات ؟');" />
                                                            </div>
                                                        </ItemTemplate>
                                                    </asp:Repeater>
                                                            ...
                                                            <ItemTemplate>
                                                                ...
                                                                <tr>
                                                                    <tr>
                                                                        ...
                                                                        <td style="text-align: center; padding: 6px 28px" runat="server" visible='<%# Convert.ToBoolean(Eval("Mark1Visable"))%>'><%# Eval("WorkMark1") %></td>
                                                                        <td style="text-align: center; padding: 6px 28px" runat="server" visible='<%# Convert.ToBoolean(Eval("Mark2Visable"))%>'><%# Eval("WorkMark2") %></td>
                                                                        <td style="text-align: center; padding: 6px 28px" runat="server" visible='<%# Convert.ToBoolean(Eval("Mark3Visable"))%>'><%# Eval("WorkMark3") %></td>
                                                                        <td style="text-align: center; padding: 6px 28px" runat="server" visible='<%# Convert.ToBoolean(Eval("Mark4Visable")) %>'><%# Eval("WorkMark4") %></td>
                                                                        <td style="text-align: center; padding: 6px 28px" runat="server" visible='<%# Convert.ToBoolean(Eval("Mark5Visable"))%>'><%# Eval("WorkMark5") %></td>
                                                                        <td style="text-align: center; padding: 6px 28px" runat="server" visible='<%# Convert.ToBoolean(Eval("Mark6Visable"))%>'><%# Eval("WorkMark6") %></td>
                                                                        <td style="text-align: center;"><span><%# Eval("TotalWorkMark") %></span></td>
