...
public partial class Attendance_ExitRegister : UserPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
     
    }
    protected void ExitLinkButton1_Click(object sender, EventArgs e)
    {
        ...
        {
            if (DataAccess.Register_Exit_Permission_TB_AddEdit(id,
                Convert.ToDateTime(((Label)ri.FindControl("Exit_Date")).Text),
                Convert.ToDateTime(((TextBox)ri.FindControl("from")).Text), Convert.ToDateTime(((TextBox)ri.FindControl("to")).Text),
              ((Label)ri.FindControl("Label4")).Text,((DropDownList)ri.FindControl("DropDownList1")).SelectedValue,
    ...
    protected void AddNew_Click(object sender, EventArgs e)
    {
        ...
        if (validate && DateValidate)
        {
            if (DataAccess.Register_Exit_Permission_TB_AddEdit("",
                Convert.ToDateTime(Date.Text),
                Convert.ToDateTime(from.Text), Convert.ToDateTime(to.Text),
                 ...
                 Convert.ToInt16(UserID), Convert.ToInt16(UserID)) > 0)
            {
                ...
                Repeater1.DataBind();
                ShowToast(this, "تم الحفظ بنجاح.", "", "success");
                EmptyControls(new WebControl[] { from, to });
    ...
    {
        Response.Redirect("~/ReportView.aspx?AllExitYear=" + TextBox1.Text + "&&AllExitMonth=" + DDMonth.SelectedValue);
    }

    protected void ExitDelete_Click(object sender, EventArgs e)
    {
        RepeaterItem ri = (RepeaterItem)((LinkButton)sender).Parent;
        string id = ((Label)ri.FindControl("Label2")).Text;
        if (DataAccess.ExitRegisterDelete(id, UserID) > 0)
        {
